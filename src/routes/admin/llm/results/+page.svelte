<script lang="ts">
  ...
  function exportColumns(): { label: string; get: (r: AdminResultsLlmRow) => string }[] {
    const cols: { label: string; get: (r: AdminResultsLlmRow) => string }[] = [
      { label: 'Model', get: r => r.hf_model_id },
    ];
    if (isVisible('dtype'))           cols.push({ label: 'Type',         get: r => r.data_type });
    if (isVisible('runtime'))         cols.push({ label: 'Framework',    get: r => r.runtime });
    ...
    if (isVisible('prompt_tokens'))   cols.push({ label: 'Prompt',       get: r => r.prompt_tokens != null ? String(r.prompt_tokens) : '' });
    if (isVisible('tokens'))          cols.push({ label: 'Output',       get: r => r.output_tokens != null ? String(r.output_tokens) : '' });
    ...
    if (isVisible('tps'))             cols.push({ label: 'TPS (t/s)',    get: r => r.tps != null ? r.tps.toFixed(1) : '' });
    if (isVisible('tpot'))            cols.push({ label: 'TPOT (ms)',    get: r => r.tpot_ms != null ? r.tpot_ms.toFixed(1) : '' });
    if (isVisible('e2e'))             cols.push({ label: 'E2E (s)',      get: r => r.e2e_ms != null ? (r.e2e_ms / 1000).toFixed(2) : '' });
    if (isVisible('e2e_tps'))         cols.push({ label: 'E2E TPS (t/s)', get: r => r.e2e_tps != null ? r.e2e_tps.toFixed(1) : '' });
    if (isVisible('compile'))         cols.push({ label: 'Compile (s)',  get: r => r.compilation_ms != null ? (r.compilation_ms / 1000).toFixed(1) : '' });
    ...
    cols.push({ label: 'User', get: r => r.user_display_name ?? r.user_id });
  ...
  function toJSON(): string {
    return JSON.stringify(sorted.map(r => {
      ...
      obj.user = r.user_display_name ?? r.user_id;
  ...
  const baseName = $derived(`admin-results-llm-${dateSuffix()}`);
    ...
    <section class="rs-layout">
      <aside class="rs-sidebar" use:autoTitle>
        <div class="sb-section">
          ...
          <input class="sb-input sb-input-text" type="text" placeholder="Model…" bind:value={filterQuery} />
        </div>

        <div class="sb-section">
          <div class="sb-section-head"><span class="sb-section-title">User</span></div>
          <div class="sb-row">
            <span class="sb-label">User</span>
            <select class="sb-input" bind:value={filterUser}>
        ...
        <div class="sb-section">
          ...
          <div class="sb-row">
            ...
            <select class="sb-input" bind:value={filterRuntime}>
              <option value="">All</option>
      ...
      <table class="results-table">
        <thead>
          <tr>
            ...
            {#if isVisible('e2e')}<th class="sortable" onclick={() => toggleSort('e2e')} title="End-to-End wall-clock time from generate() call to the last token. Lower is better. Includes prefill + decode.">E2E{sortIndicator('e2e')}</th>{/if}
            ...
            {#if isVisible('compile')}<th class="sortable" onclick={() => toggleSort('compile')} title="Initial model compilation/load-and-compile time. One-time cost per session.">Compile{sortIndicator('compile')}</th>{/if}
