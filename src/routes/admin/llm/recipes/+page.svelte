...
<div class="admin-recipes">
  ...
  <section class="curation-section">
    ...
    {#if featuredList.length === 0}
      <p class="empty">No featured LLM recipes. Feature some from the list below.</p>
    {:else}
        ...
        <tbody>
          {#each featuredList as recipe (recipe.id)}
            <tr>
              ...
              <td class="cell-name">
                <a href="/llm/recipe/{recipe.slug}" class="name-link">{recipe.name}</a>
              </td>
              ...
              <td class="cell-action">
                <form method="POST" action="?/unfeatureRecipe" use:enhance={() => {
                  return async ({ update }) => { onUnfeature(recipe.id); await update({ reset: false }); };
                }}>
                  <input type="hidden" name="id" value={recipe.id} />
  ...
  <section class="curation-section">
    ...
    <input
      type="search"
      class="search-input"
      placeholder="Search community LLM recipes…"
      bind:value={communitySearch}
      aria-label="Search community LLM recipes"
    />
    {#if filteredUnfeatured.length === 0}
      <p class="empty">{communitySearch ? 'No matches.' : 'All public LLM recipes are featured.'}</p>
    {:else}
      <table class="curation-table">
        <thead>
          <tr>
            ...
            <tr>
              ...
              <td class="cell-action">
                <form method="POST" action="?/featureRecipe" use:enhance={() => {
                  return async ({ update }) => { onFeature(recipe.id); await update({ reset: false }); };
                }}>
                  <input type="hidden" name="id" value={recipe.id} />
...
<style>
  ...
  .curation-section { margin-bottom: var(--space-5); }
  ...
  .curation-table tbody tr:hover td { background: var(--color-surface-sunken); }
  ...
  .order-input:focus-visible { border-color: var(--color-focus-ring); outline: none; }
  ...
  .cell-date { font-size: var(--text-xs); color: var(--color-text-muted); white-space: nowrap; }
  ...
  .action-feature { color: var(--color-primary); border-color: var(--color-primary); }
  .action-feature:hover { background: var(--color-accent-light); }

  .action-unfeature { color: var(--color-text-muted); }
  .action-unfeature:hover { color: var(--color-error); border-color: var(--color-error); }
