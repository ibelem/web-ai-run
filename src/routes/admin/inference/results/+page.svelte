<script lang="ts">
  ...
  function exportColumns(): { label: string; get: (r: AdminResultRow) => string }[] {
    const cols: { label: string; get: (r: AdminResultRow) => string }[] = [
  ...
  const baseName = $derived(`admin-results-${dateSuffix()}`);
