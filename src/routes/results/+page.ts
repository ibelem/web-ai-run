...
export const load: PageLoad = async () => {
  ...
  if (!session) {
    return { results: [], distinctBackends: [], distinctDataTypes: [], distinctStatuses: [], distinctWebnnEps: [], distinctFrameworks: [], profile: null, error: null };
  }

  const userId = session.user.id;

  // Fetch distinct filter values (no limit) + profile in parallel
  const [backendRows, dtRows, statusRows, epRows, fwRows, profileRes, resultsRes] = await Promise.all([
    (supabase.from('results') as any).select('backend').eq('user_id', userId),
    (supabase.from('results') as any).select('data_type').eq('user_id', userId),
    (supabase.from('results') as any).select('status').eq('user_id', userId),
    (supabase.from('results') as any).select('webnn_ep').eq('user_id', userId).neq('webnn_ep', ''),
    (supabase.from('results') as any).select('ort_version, litert_version').eq('user_id', userId),
  ...
  const distinctBackends: string[] = [...new Set((backendRows.data ?? []).map((r: any) => r.backend))].sort() as string[];
  const distinctDataTypes: string[] = [...new Set((dtRows.data ?? []).map((r: any) => r.data_type))].sort() as string[];
  const distinctStatuses: string[] = [...new Set((statusRows.data ?? []).map((r: any) => r.status))].sort() as string[];
  const distinctWebnnEps: string[] = [...new Set((epRows.data ?? []).map((r: any) => r.webnn_ep).filter(Boolean))].sort() as string[];
