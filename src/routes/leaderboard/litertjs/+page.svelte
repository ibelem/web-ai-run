<script lang="ts">
  ...
  const versions = $derived(
    [...new Set(data.results.map((r: LitertRow) => r.litert_version))].sort().reverse()
  );

  const backends = $derived(
    [...new Set(data.results.map((r: LitertRow) => r.backend))].sort()
  );

  const dataTypes = $derived(
    [...new Set(data.results.map((r: LitertRow) => r.data_type))].sort()
  );

  const webnnEps = $derived(
    [...new Set(data.results.map((r: LitertRow) => r.webnn_ep).filter(Boolean))].sort()
  );
  ...
  const compareRows = $derived.by(() => {
    ...
    return rows.sort((a, b) => a.model_id.localeCompare(b.model_id) || a.file_path.localeCompare(b.file_path));
  });
        ...
        <table class="compare-table">
          <thead>
            <tr>
              ...
              <th class="th-backend">Backend</th>
              <th class="th-dtype">Type</th>
              <th class="th-metric">{versionA}<br><span class="th-metric-label">{metricLabel}</span></th>
              <th class="th-metric">{versionB}<br><span class="th-metric-label">{metricLabel}</span></th>
