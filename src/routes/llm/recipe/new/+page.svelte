<script lang="ts">
  ...
  const canSave = $derived(recipeName.trim().length > 0 && models.length > 0);
  ...
  async function save(andRun = false) {
    ...
    if (models.length === 0) { errorMsg = 'Add at least one LLM'; return; }
    ...
    if (andRun) {
      sessionStorage.setItem('llm_recipe_models', JSON.stringify(models));
      goto('/llm/run');
    } else {
      goto('/llm/recipe');
    }
...
<aside class="recipe-sidebar">
  <div class="sidebar-header">
    ...
    <a href="/llm/recipe" class="sidebar-back">All</a>
  </div>
  <ul class="sidebar-list">
    ...
    {#each (data.recipes ?? []) as r (r.id)}
      <li>
        <a href="/llm/recipe/{r.slug}/edit" class="sidebar-item">
...
<div class="edit-page">
  <header class="page-header">
    <h1>{data.forkName ? 'Fork LLM Recipe' : 'New LLM Recipe'}</h1>
    {#if data.forkName}
  ...
  <section class="zone">
    ...
    {#if models.length === 0}
      <div class="empty-models"><p>No models yet. Search Hugging Face below to add some.</p></div>
    {:else}
      <ul class="model-list">
        {#each models as m, i}
  ...
  <section class="zone">
    ...
    <p class="zone-hint">Only text-generation repos are shown. Click a dtype chip to add it to your recipe.</p>
    <div class="search-wrap">
      ...
      <input
        class="search-input"
        type="text"
        placeholder="Search LLM repos (e.g. Phi-4, Qwen4, SmolLM)…"
  ...
  <div class="save-bar">
    {#if !canSave}
      <p class="save-hint">
        {#if !recipeName.trim() && models.length === 0}
    ...
    <div class="save-actions">
      ...
      {/if}
      <a href="/llm/recipe" class="btn-ghost">Cancel</a>
      <button class="btn-ghost" onclick={() => save(true)} disabled={saving || !canSave}>
        Save &amp; Run →
      </button>
      <button class="btn-save" onclick={() => save(false)} disabled={saving || !canSave}>
