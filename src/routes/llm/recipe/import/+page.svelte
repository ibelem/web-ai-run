  ...
  async function doImport() {
    ...
    if (mode === 'merge') {
      ...
      if (result?.data?.success) {
        ...
        setTimeout(() => goto(`/llm/recipe/${result.data.slug}`), 1500);
      }
    } else {
      ...
      if (result?.data?.slug) goto(`/llm/recipe/${result.data.slug}`);
    }
  }
</script>
...
<div class="import-page">
  <header class="page-header">
    ...
    <a href="/llm/recipe" class="btn-cancel">Cancel</a>
  </header>
      ...
      <section class="zone">
        ...
        <div class="field-row">
          {#if data.myRecipes.length === 0}
            <p class="no-recipes">You don't have any LLM recipes yet. <a href="/llm/recipe/new">Create one first.</a></p>
          {:else}
